...
public class ProductRepositoryWithBagRelationshipsImpl implements ProductRepositoryWithBagRelationships {
    ...
    public Optional<Product> fetchBagRelationships(Optional<Product> product) {
        return product.map(this::fetchWarehouses);
    }
    ...
    public List<Product> fetchBagRelationships(List<Product> products) {
        return Optional.of(products).map(this::fetchWarehouses).orElse(Collections.emptyList());
    }

    Product fetchWarehouses(Product result) {
        return entityManager
            .createQuery("select product from Product product left join fetch product.warehouses where product.id = :id", Product.class)
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    }

    List<Product> fetchWarehouses(List<Product> products) {
        ...
        List<Product> result = entityManager
            .createQuery("select product from Product product left join fetch product.warehouses where product in :products", Product.class)
